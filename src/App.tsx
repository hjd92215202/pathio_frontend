            ...
            <div className="relative z-10">
              ...
              <div className="space-y-4 mb-8">
                {['无限路线图空间', '不限数量的协作席位', '高级分享权限', '专属技术支持'].map(f => (
                  <div key={f} className="flex items-center gap-3 text-xs font-bold opacity-90 text-white">
...
// 通用 Dialog - 已修复 transition-all 重复的警告
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200 text-slate-900">
        ...
        {type === 'input' && (
          <input autoFocus className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-pathio-500 outline-none mb-8 font-bold transition-all text-slate-800" placeholder={placeholder} value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && onConfirm(val)} />
        )}
        ...
        <div className="flex gap-3">
          ...
          {/* 💡 修复点：移除了表达式内部重复的 transition-all 类名 */}
          <button 
...
function CanvasViewport({ roadmapId, onToggleSidebar, isSidebarCollapsed, setDialog, setGlobalOrgName }: any) {
  ...
  // 💡 修复：每当 roadmapId 变化，强制滑回顶部并清理菜单，解决管理页不可见问题
  useEffect(() => {
    setActiveNode(null); 
    setMenu(null);
  }, [roadmapId]);
  ...
  const onPaneClick = useCallback((e: React.MouseEvent) => {
    ...
    setDialog({ isOpen: true, title: '新建研究节点', type: 'input', defaultValue: '新节点', onConfirm: (title: string) => { if (!title) return; api.post('/nodes', { title, pos_x: pos.x, pos_y: pos.y, roadmap_id: roadmapId }).then(res => { setNodes(nds => nds.concat({ id: res.data.id, type: 'pathio', position: { x: res.data.pos_x, y: res.data.pos_y }, data: { label: res.data.title, status: 'todo' } })); setDialog({ isOpen: false }); }).catch(err => { if (err.response?.status === 402) { setDialog({ isOpen: false }); return; } }); } });
  ...
  return (
    <div className={`flex-1 relative bg-white overflow-hidden transition-all duration-500 shadow-2xl ${isSidebarCollapsed ? 'rounded-none' : 'rounded-l-[2.5rem]'}`} style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden', outline: '1px solid transparent' }}>
      ...
      <div className="w-full h-[200vh] transition-transform duration-700 cubic-bezier(0.65, 0, 0.35, 1)" style={{ transform: activeNode ? 'translateY(-50%)' : 'translateY(0%)', willChange: 'transform' }}>
        ...
        <div className="w-full h-screen bg-white overflow-hidden">{activeNode && <NoteView nodeId={activeNode.id} nodeTitle={activeNode.title} onBack={() => setActiveNode(null)} nodes={nodes} edges={edges} />}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    if (authToken) { api.get('/org/details').then(res => setOrgName(res.data.name)).catch(() => {}); }
  }, [authToken]);
