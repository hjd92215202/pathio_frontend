...
import { api } from '../api';
import type { DialogConfig, RoadmapSummary } from '../types';
...
export default function Sidebar({
  ...
  const fetchRoadmaps = useCallback(() => {
    api.get<RoadmapSummary[]>('/roadmaps').then((res) => {
  ...
  const handleRenameRoadmap = (event: React.MouseEvent, id: string, oldTitle: string) => {
    ...
    setDialog({
      ...
      onConfirm: (newTitle?: string) => {
        ...
        if (nextTitle && nextTitle !== oldTitle) {
          api.put(`/roadmaps/${id}`, { title: nextTitle }).then(() => {
            fetchRoadmaps();
            setDialog((prev) => ({ ...prev, isOpen: false }));
          });
        } else {
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pr-1 space-y-2 custom-scrollbar min-w-[200px]">
        ...
        <nav className="space-y-1">
          {roadmaps.map((roadmap) => (
            <button key={roadmap.id} onClick={() => onSelect(roadmap.id)} onContextMenu={(event) => handleRenameRoadmap(event, roadmap.id, roadmap.title)} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 whitespace-nowrap flex items-center gap-3 ${currentId === roadmap.id ? 'bg-pathio-500 text-white shadow-lg shadow-pathio-500/20 translate-x-1' : 'text-gray-400 hover:bg-gray-800/50 hover:text-gray-200'}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${currentId === roadmap.id ? 'bg-white' : 'bg-gray-700'}`}></div>
